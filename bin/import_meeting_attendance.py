...
from pmg import db
from pmg.models.resources import (
    CommitteeMeeting,
    Member,
    Committee,
    House,
...
missing_members = ["August, Ms C", "Bernard, Mr J"]
...
def get_committee(committee_name):
    ...
    return Committee.query.filter(
        Committee.name == committee_name_map[committee_name]
    ).first()
...
if __name__ == "__main__":
    ...
    with open(args.log, "wb") as logfile:
        with open(args.input) as csvfile:
            ...
            attendance_check = [
                (att.meeting_id, att.member_id) for att in existing_attendance
            ]
            ...
            for row in reader:
                ...
                if reader.line_num >= 0:
                    if len(meeting_count[(row["Date"], row["Name Committee"])]) > 1:
                        ...
                        error = (
                            "Multiple committee meetings in a day found in import data."
                        )
                    ...
                    if committee:
                        if (
                            committee.name,
                            meeting_date.date(),
                        ) in committee_meeting_dict:
                            committee_meeting_results = committee_meeting_dict[
                                (committee.name, meeting_date.date())
                            ]
                        else:
                            committee_meeting_results = (
                                CommitteeMeeting.query.filter(
                                    CommitteeMeeting.committee == committee
                                )
                                .filter(
                                    func.date(CommitteeMeeting.date)
                                    == meeting_date.date()
                                )
                                .all()
                            )
                    ...
                    if len(committee_meeting_results) != 1:
                        ...
                        log_error(writer, row, error=error)
                        print(
                            "Meeting error: %s :: No of meetings: %s"
                            % (
                                str(reader.line_num),
                                str(len(committee_meeting_results)),
                            )
                        )
                    else:
                        ...
                        if not (committee_meeting.id, member.id) in meeting_attendance:
                            ...
                            log_error(
                                writer,
                                row,
                                error="Duplicate attendance for meeting in sheet.",
                            )
                            continue
