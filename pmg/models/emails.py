...
class SavedSearch(db.Model):
    ...
    committee_id = db.Column(
        db.Integer, db.ForeignKey("committee.id", ondelete="CASCADE")
    )
    ...
    def send_all_alerts(cls):
        ...
        searches = (
            SavedSearch.query.join(User).filter(User.confirmed_at != None).all()
        )  # noqa
...
def send_sendgrid_email(
    subject, from_name, from_email, recipient_users, html, utm_campaign
):
    ...
    message.add_filter(
        "templates", "template_id", app.config["SENDGRID_TRANSACTIONAL_TEMPLATE_ID"]
    )

    message.smtpapi.set_substitutions(
        {"*|NAME|*": [r.name or "Subscriber" for r in recipient_users]}
    )
    ...
    log.info(
        'Email will be sent to %d recipients, campaign=%s subject="%s"'
        % (len(recipients), utm_campaign, subject)
    )
