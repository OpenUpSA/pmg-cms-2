...
class TestCommitteeMeetingsAPI(PMGTestCase):
    ...
    def tearDown(self):
        self.fx.teardown()
        super(TestCommitteeMeetingsAPI, self).tearDown()

    def test_premium_committee_meeting(self):
        res = self.client.get(
            "/committee-meeting/%s/" % self.fx.CommitteeMeetingData.premium_recent.id,
    ...
    def test_premium_committee_meeting_old(self):
        res = self.client.get(
            "/committee-meeting/%s/" % self.fx.CommitteeMeetingData.premium_old.id,
            base_url="http://api.pmg.test:5000/",
        )
        self.assertEqual(200, res.status_code)
