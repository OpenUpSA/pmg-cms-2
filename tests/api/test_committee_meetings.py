...
class TestCommitteeMeetingsAPI(PMGTestCase):
    ...
    def test_premium_committee_meeting(self):
        res = self.client.get('/committee-meeting/%s/' % self.fx.CommitteeMeetingData.premium_recent.id, base_url='http://api.pmg.dev:5000/')
        self.assertEqual(200, res.status_code)
        self.assertTrue(res.json['premium_content_excluded'])

    def test_premium_committee_meeting_old(self):
        res = self.client.get('/committee-meeting/%s/' % self.fx.CommitteeMeetingData.premium_old.id, base_url='http://api.pmg.dev:5000/')
        self.assertEqual(200, res.status_code)
