...
class Petition(ApiResource, db.Model):
    ...
    # Override the default slug_prefix to use plural form
    slug_prefix = "petitions"
...
class PetitionEvent(db.Model):
    ...
    # Flag to mark events that are automatically generated by the system
    # (e.g., from hansard linkages) and should not be manually editable
    system_generated = db.Column(db.Boolean, nullable=False, default=False)

    # Optional committee report reference for report-related events
    committee_report_id = db.Column(
        db.Integer,
        db.ForeignKey("tabled_committee_report.id", ondelete="SET NULL"),
        nullable=True,
        index=True
    )
